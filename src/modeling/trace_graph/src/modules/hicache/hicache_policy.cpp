/**
 * @file
 * @brief HiCache target policy helpers。
 */
...
HiCacheResolvedPolicyState resolve_hicache_policy(const HiCacheConfig & config) {
    ...
    HiCacheResolvedPolicyState resolved{
        ...
        .prefetch_timeout_rule = "base + per_ki_token * token_count / 1024, capped by max when max > 0",
        .prefetch_rate_limit_rule = "active_requested_pages >= prefetch_capacity_limit_pages",
        .terminal_prefetch_checkpoint_kinds = { "complete", "completed", "terminate", "terminated", "final", "finalize" },
    };
...
bool HiCachePolicy::terminal_prefetch_checkpoint(const std::string & check_kind) const {
    const auto kind = lower_copy(check_kind);
...
bool HiCachePolicy::prefetch_timeout_elapsed(uint64_t enqueue_ts, uint64_t checkpoint_ts, uint64_t token_count) const {
    ...
    double timeout_sec = resolved_.prefetch_timeout_base_sec + resolved_.prefetch_timeout_per_ki_token_sec * static_cast<double>(token_count) / 1024.0;
    if (resolved_.prefetch_timeout_max_sec > 0.0) timeout_sec = std::min(timeout_sec, resolved_.prefetch_timeout_max_sec);
