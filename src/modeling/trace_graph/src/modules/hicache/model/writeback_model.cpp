...
/**
 * @brief 在下一条 target-control fact 前释放 write-through backup 临时 ref。
 *
 * SGLang write-through-selective backup 在 CPU 写入 ACK 前会持有普通 lock ref。
 * 模型把 ACK 时序折叠到下一条 target control fact 开始处 drain，最后一条 fact 后
 * 仍未 drain 的 ref 可保留到 final-state。
 */
