...
Json target_config_json(const HiCacheConfig & config) {
    return {
...
Json resolved_policy_json(const HiCacheResolvedPolicyState & policy) {
    return {
        ...
        { "terminal_prefetch_checkpoint_kinds", policy.terminal_prefetch_checkpoint_kinds },
...
std::string HiCacheSummary::to_json() const {
    Json root;
    root["status"] = status;
    root["target_config"] = target_config_json(target_config);
    root["resolved_policy"] = resolved_policy_json(resolved_policy);
    ...
    root["active_ref_owner_count"] = active_ref_owner_count;
    root["radix_split_count"] = radix_split_count;
    root["control_checkpoint_count"] = control_checkpoint_count;
    root["async_lifecycle_transition_count"] = async_lifecycle_transition_count;
    root["policy_decision_count"] = policy_decision_count;
    root["storage_known_page_count"] = storage_known_page_count;
    root["storage_readable_page_count"] = storage_readable_page_count;
    root["storage_backend_readable_count"] = storage_backend_readable_count;
    root["storage_materialized_page_count"] = storage_materialized_page_count;
    root["capacity_mutation_count"] = capacity_mutation_count;
    root["capacity_victim_choice_count"] = capacity_victim_choice_count;
    ...
    root["control_checkpoint_trace"] = array_from(control_checkpoint_trace, control_checkpoint_json);
    root["async_lifecycle_trace"] = array_from(async_lifecycle_trace, async_lifecycle_json);
    root["policy_decision_trace"] = array_from(policy_decision_trace, policy_decision_json);
    root["capacity_mutation_trace"] = array_from(capacity_mutation_trace, capacity_mutation_json);
    root["capacity_victim_choices"] = array_from(capacity_victim_choices, capacity_victim_choice_json);
    root["capacity_audit_issues"] = array_from(capacity_audit_issues, capacity_audit_issue_json);
    root["ref_mutation_trace"] = array_from(ref_mutation_trace, ref_mutation_json);
    root["ref_audit_issues"] = array_from(ref_audit_issues, ref_audit_issue_json);
    root["final_state"] = final_state_json(*this);
    root["storage_directory_inclusive_state"] = derived_snapshot_json(storage_directory_inclusive_state);
