...
[[nodiscard]] std::vector<const HiCacheSourceFactNode *> nested_storage_services(const HiCacheSourceDagIndex & source, const HiCacheSourceFactNode & timing,
...
[[nodiscard]] const HiCacheSourceFactNode * request_consumer(const HiCacheSourceDagIndex & source, std::string_view request_id, std::string_view pid,
                                                             uint64_t source_end_us);
[[nodiscard]] const HiCacheSourceFactNode * request_role_before(const HiCacheSourceDagIndex & source, std::string_view request_id, std::string_view pid,
...
[[nodiscard]] HiCacheIoOperationRecord build_record(const HiCacheSourceDagIndex & source, const HiCacheSourceFactNode & timing, HiCacheIoOperationKind kind);
} // namespace markov::trace_graph::modules::hicache::patch::io_operation_ledger_detail
