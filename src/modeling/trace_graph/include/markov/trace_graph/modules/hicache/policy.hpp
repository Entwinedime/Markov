/**
 * @file
 * @brief HiCache target policy 解析结果和纯 policy gate。
 */
#pragma once
...
class HiCachePolicy {
public:
    explicit HiCachePolicy(frontend::HiCacheConfig config = frontend::HiCacheConfig{});

    /** @brief 展开后的 target policy 状态；只读暴露给 summary/诊断。 */
    [[nodiscard]] const HiCacheResolvedPolicyState & resolved() const { return resolved_; }

    /** @brief write-through-selective 进入 backup 的 request 计数阈值。 */
    [[nodiscard]] uint64_t write_through_threshold() const;

    /** @brief 当前 write policy 是否需要按命中计数判断 backup。 */
    [[nodiscard]] bool write_count_enabled() const;

    /** @brief 当前 write policy 是否采用 write-back。 */
    [[nodiscard]] bool write_back_enabled() const;

    /** @brief prefetch stop policy 使用的最小 page threshold。 */
    [[nodiscard]] uint64_t prefetch_threshold_pages() const;

    /** @brief active prefetch reservation 的容量上限。 */
...
[[nodiscard]] HiCacheResolvedPolicyState resolve_hicache_policy(const frontend::HiCacheConfig & config);
