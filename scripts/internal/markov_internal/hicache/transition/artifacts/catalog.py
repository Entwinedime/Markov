...
from ..validation.taxonomy import aggregate_transition_families
...
def write_transition_catalog_outputs(
    matrix_dir: Path, output_path: Path, catalog: dict[str, Any], *, sample_limit: int
) -> None:
...
def write_transition_catalog_markdown(path: Path, catalog: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "Status: generated diagnostic artifact. Produced by `scripts/internal/entrypoints/hicache_transition.py --mode compare-matrix --emit-catalog`.",
        "",
        "## Summary",
        "",
