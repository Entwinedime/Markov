...
from ...matrix.runs.types import safe_slug
    ...
    *,
    ...
    for matrix_row_path in matrix_row_paths:
        ...
        comparison_path = prediction_dir / (
            "transition_exactness_self.json" if matrix_row.get("is_self") else "transition_exactness_cross.json"
        )
        should_rebuild_comparison = force or emit_catalog or not comparison_path.is_file()
        comparison_can_run = (
            should_rebuild_comparison
            and prediction_paths.predicted_trace.is_file()
            and (observed_path.is_file() or target_run is not None)
        )
...
def summarize_matrix_transition_row(
    matrix_row: dict[str, Any], comparison_path: Path, observed_path: Path
) -> dict[str, Any]:
