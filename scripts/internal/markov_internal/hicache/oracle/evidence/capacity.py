...
def build_hicache_capacity_config_audit(
    ...
    comparisons = {
        "page_size": _compare_int_config(
            "page_size", target["page_size"], _unique_int_values(unique_values, ["page_size"])
        ),
        "write_policy": _compare_policy_config(
            "write_policy", target["write_policy"], _unique_policy_values(unique_values, ["write_policy"])
        ),
        "prefetch_policy": _compare_policy_config(
            "prefetch_policy", target["prefetch_policy"], _unique_policy_values(unique_values, ["prefetch_policy"])
        ),
...
def recommend_hicache_target_config(
    ...
    prefetch_threshold_values = _unique_int_values(
