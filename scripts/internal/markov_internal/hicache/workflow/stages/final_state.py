...
from ...matrix.predictions.prediction import (
    ACTIVE_STATE_TIERS,
    matrix_summary,
...
from ...matrix.runs.discovery import build_prediction_specs
from ...matrix.runs.types import PredictionSpec
...
def build_quality_gate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "runs": {str(row.get("run_id")): row for row in runs if isinstance(row, dict) and row.get("run_id")},
...
def skip_prediction_reason(spec: PredictionSpec, quality_gate: dict[str, Any]) -> str:
    ...
    if isinstance(source_quality, dict) and not source_quality.get("workflow_input_ready"):
        return "source_workflow_input_not_ready"
    if isinstance(target_quality, dict) and not target_quality.get("workflow_input_ready"):
...
def run_or_summarize_prediction(
    ...
    if skip_reason:
        row = build_skipped_prediction_row(spec, output_dir, skip_reason)
    ...
    row = build_prediction_row(spec, output_dir, return_code, elapsed_sec)
    write_json(pred_dir / "matrix_row.json", row)
    return row
...
def build_model_command(spec: PredictionSpec, pred_dir: Path, config_path: Path) -> list[str]:
    ...
    validation backend、module summary 和 validation 输出由 workflow runner config 控制；
    命令行只传本次 prediction 的 source/target 路径，避免同一开关出现两个来源。
    """
...
def summarize_existing_predictions(
    ...
    *,
    ...
    rows = [
        collect_prediction_row(spec, output_dir) for spec in prediction_specs_for_options(runs, options, scope=scope)
    ]
...
def collect_prediction_row(spec: PredictionSpec, output_dir: Path) -> dict[str, Any]:
    ...
    return build_prediction_row(spec, output_dir, return_code=0, elapsed_sec=0.0)
