...
from .types import PredictionSpec, ProfileRun
...
def profile_run_from_manifest(manifest_path: Path) -> ProfileRun:
    ...
    config_id = required_metadata_value(metadata, "suite_server_id", config_path)
    input_id = required_metadata_value(metadata, "suite_input_id", config_path)
...
def required_metadata_value(metadata: dict[str, Any], key: str, config_path: Path) -> str:
    """读取当前 suite 合同要求的 metadata 字段。"""

    value = metadata.get(key)
    if isinstance(value, str) and value.strip():
        return value.strip()
    raise ValueError(f"missing metadata.{key} in {config_path}")
...
def filter_runs(
    ...
    *,
    ...
    return [
        run
        for run in runs
        if (not input_ids or run.input_id in input_ids) and (not config_filter or run.config_id in config_filter)
    ]
...
def build_prediction_specs(
    ...
    *,
    ...
    for _input_id, by_config in group_runs_by_input(runs).items():
        sources = [
            run for config_id, run in by_config.items() if not source_config_ids or config_id in source_config_ids
        ]
        targets = [
            run for config_id, run in by_config.items() if not target_config_ids or config_id in target_config_ids
        ]
