...
class DagDiagnosticsValidation(DagArtifactValidation):
    ...
    def _base_row(result: ModelRunResult) -> dict[str, Any]:
        ...
        blockers.extend(str(blocker) for blocker in faithful_blockers)
        if faithful and faithful.get("ready") is not True and not faithful_blockers:
            blockers.append("faithful_replay_not_ready")
        return {
