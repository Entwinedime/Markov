...
任务被图变换删除后，后续任务的剩余就绪延迟仍按原始线程前驱测量，不能从删除后的节点序列重新估算；
否则被删任务的执行时间会被错误地加回为 gap。当前活跃任务只决定预测中的排队与资源占用。
...
建图生成的 control `.self` 是外层区间扣除显式子调用后的余量，不等于纯 CPU 工作。
同线程 CPU 通信的进入/返回位置会将这些余量分段，保留全部时间；真实执行叶子不被裁剪。
这只提供后续等待接管的边界，不会自动删除通信等待或改变成本。
逐层等待的原始 NPU 计数器由 manifest 中对应 Torch trace 的时钟元数据转换为纳秒，之后才可与 CPU 叶子核对边界；
缺少换算时不能将计数器当成墙钟，旧墙钟观测也不被自动升级为已对齐数据。当前仍未接入逐层等待的增删变换。
阶段观察与 HiCache scope 共用逐层调用归属：按纳秒调用位置、同线程叶子及 enqueue→worker→wait→Record
依赖确认的等待提交不再计入 Prefill/Decode 的 CPU 成本。它们仍保留在完整图中，由 HiCache 接管，并未被清零。
未启用的短调用保留精确插入位置；缺时钟、缺叶子或依赖歧义通过 `layer_wait_status/issues` 报告，不能授权节点删除。
调用位置使用独立的纳秒排序视图，不改变建图时的 CPU 节点顺序，否则同一微秒内的节点可能把 gap 归给错误的 owner。
这也会改变新数据的分项归属，不能拿新旧分项范围的差额宣称完整 E2E 精度改善。
